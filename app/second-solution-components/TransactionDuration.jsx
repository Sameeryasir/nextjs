...
const SelectBranchModal = ({ isOpen, onClose, onConfirm }) => {
  ...
  return (
    <div onClick={(e) => e.stopPropagation()} className={`fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 bg-white rounded-lg shadow-2xl flex flex-col w-full max-w-4xl`}>
      <div className="flex justify-between items-center bg-gray-100 border-b py-2 px-4 rounded-t-lg"><h3 className="text-lg font-semibold text-gray-900">Select A Branch</h3><div className="flex items-center space-x-2"><button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X size={20} /></button></div></div>
      <div className="p-6 space-y-4">
        <div className="flex items-center gap-4 text-sm pb-4"><label className="font-medium text-gray-600">Code</label><input type="text" className="w-48 px-3 py-2 border border-gray-300 rounded-md" /><label className="font-medium text-gray-600">Name</label><input type="text" className="w-48 px-3 py-2 border border-gray-300 rounded-md" /><button className="px-6 py-2 bg-[#FF9900] text-white font-semibold rounded-md hover:brightness-95 transition">Search</button></div>
        <PaginationControls />
        <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="bg-[#FF9900] text-white"><tr>{modalTableHeaders.map(header => <th key={header} className="p-3 text-left font-semibold">{header}</th>)}</tr></thead><tbody>{modalTableData.map((row, index) => (<tr key={index} className="bg-white hover:bg-gray-100 border-b"><td className="p-3 text-gray-700">{row.code}</td><td className="p-3 text-gray-700">{row.date}</td><td className="p-3 text-gray-700">{row.type}</td><td className="p-3 text-gray-700">{row.description}</td><td className="p-3 text-gray-700">{row.department}</td></tr>))}</tbody></table></div>
      </div>
      <div className="flex justify-end items-center p-4 space-x-3 bg-gray-100 border-t"><button onClick={onConfirm} className="px-8 py-2 bg-[#FF9900] text-white font-bold rounded-md hover:brightness-95 transition">OK</button><button onClick={onClose} className="px-8 py-2 bg-gray-800 text-white font-bold rounded-md hover:bg-gray-700 transition">Cancel</button></div>
    </div>
  );
};
...
const SelectVendingServerModal = ({ isOpen, onClose, onConfirm }) => {
  ...
  return (
    <div onClick={(e) => e.stopPropagation()} className={`fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 bg-white rounded-lg shadow-2xl flex flex-col w-full max-w-3xl`}>
      <div className="flex justify-between items-center bg-gray-100 border-b py-2 px-4 rounded-t-lg"><h3 className="text-lg font-semibold text-gray-900">Select Vending Server</h3><div className="flex items-center space-x-2"><button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X size={20} /></button></div></div>
      <div className="p-6 space-y-4"><div className="flex items-center gap-4 text-sm pb-4"><label className="font-medium text-gray-600">Code</label><input type="text" className="w-48 px-3 py-2 border border-gray-300 rounded-md" /><label className="font-medium text-gray-600">Name</label><input type="text" className="w-48 px-3 py-2 border border-gray-300 rounded-md" /><button className="px-6 py-2 bg-[#FF9900] text-white font-semibold rounded-md hover:brightness-95 transition">Search</button></div><PaginationControls /><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="bg-[#FF9900] text-white"><tr>{modalTableHeaders.map(header => <th key={header} className="p-3 text-left font-semibold">{header}</th>)}</tr></thead><tbody>{modalTableData.map((row, index) => (<tr key={index} className="bg-white hover:bg-gray-100 border-b"><td className="p-3 text-gray-700">{row.code}</td><td className="p-3 text-gray-700">{row.description}</td><td className="p-3 text-gray-700">{row.region}</td><td className="p-3 text-gray-700">{row.serverIp}</td></tr>))}</tbody></table></div></div>
      <div className="flex justify-end items-center p-4 space-x-3 bg-gray-100 border-t"><button onClick={onConfirm} className="px-8 py-2 bg-[#FF9900] text-white font-bold rounded-md hover:brightness-95 transition">OK</button><button onClick={onClose} className="px-8 py-2 bg-gray-800 text-white font-bold rounded-md hover:bg-gray-700 transition">Cancel</button></div>
    </div>
  );
};

// --- Main Transactions Duration Component ---
const TransactionsDuration = () => {
  const [isBranchModalOpen, setIsBranchModalOpen] = useState(false);
  const [isVendingServerModalOpen, setIsVendingServerModalOpen] = useState(false);
  const tableHeaders = ["3RD Party", "Vending Server", "TransID", "Meter Num", "Star Time", "End Time", "Result", "Fail Code"];
    ...
    <>
      <div className="p-6 w-full bg-white min-h-screen">
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-800">Transaction Duration</h1>
        ...
        <div className="space-y-3 mb-6 max-w-3xl">
            ...
            <div className="flex items-center gap-x-6">
                <label className="w-32 text-sm font-medium text-gray-600 text-right">Trans Result</label>
                <select className="flex-grow px-3 py-2 border border-gray-300 rounded-md bg-white"><option></option></select>
            </div>
            <div className="flex items-center gap-x-6">
                <label className="w-32 text-sm font-medium text-gray-600 text-right">Type</label>
                <select className="flex-grow px-3 py-2 border border-gray-300 rounded-md bg-white"><option></option></select>
            </div>
             <div className="flex items-center gap-x-6">
...
export default TransactionsDuration;
