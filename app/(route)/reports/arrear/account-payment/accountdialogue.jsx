...
/**
 * A modal dialogue for searching, selecting, and confirming a branch.
 * @param {object} props - The component props.
 * @param {Function} props.onClose - Function to call when the modal should be closed.
 * @param {Function} props.onSelect - Function to call with the selected branch data.
 */
function BranchSelectionDialogue({ onClose, onSelect }) {
  const router = useRouter();

  // State for search input fields
  const [search, setSearch] = useState({ code: "", name: "" });
  // State for API data, pagination, loading status, and errors
  ...
  // --- API Call ---
  // Fetches branch data based on search criteria and pagination
  const fetchBranches = async (page = 1) => {
    ...
    try {
      ...
      setData({ rows: [], total: 0, totalPages: 1, pageIndex: 1 }); // Reset data on error
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSearchClick = () => {
    fetchBranches(1); // Reset to page 1 for a new search
  };
  
  // Confirms selection and passes data to the parent component
  const handleOkClick = () => {
    if (selectedRow) {
  ...
  const handlePageChange = (newPage) => {
    ...
    if (pageNum >= 1 && pageNum <= data.totalPages) {
      fetchBranches(pageNum);
    }
  };
  
  // Allows quick selection by double-clicking a row
  const handleRowDoubleClick = (row) => {
    onSelect(row);
    onClose();
  };
  
  const tableHeaders = ["Code", "Date", "Types", "Description", "Department", "SCC"];
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl p-4 flex flex-col max-h-[90vh]">
        ...
        <div className="flex justify-between items-center border-b pb-2 mb-4">
          <h2 className="text-xl font-semibold">Select the Branch</h2>
        ...
        <div className="flex items-center gap-2 mb-2">
            ...
            <input
                ...
                value={search.name} onChange={handleInputChange}
        ...
        <div className="flex flex-wrap items-center gap-2 bg-gray-100 px-3 py-1 rounded text-sm mb-4">
            ...
            <span>
              Total {data.total} records, record {loading || data.rows.length === 0 ? 0 : ((data.pageIndex - 1) * 10) + 1} - {Math.min(data.pageIndex * 10, data.total)}, page {data.pageIndex}/{data.totalPages || 1}
            </span>
             <div className="flex items-center">
             ...
             <div className="flex items-center gap-2 ml-4">
              ...
              <input
                  ...
                  onKeyDown={(e) => {
                      if (e.key === 'Enter') handlePageChange(e.target.value);
                  }}
              ...
              {loading ? (
                ...
                data.rows.map((row) => (
                  ...
                  >
                    ...
                    <td className="p-2 border-t">{row.RegDate}</td>
                    ...
                    <td className="p-2 border-t">{row.SGC}</td>
                  </tr>
                ))
              )}
            </tbody>
...
// You can keep the default export name if needed for your project structure.
export default BranchSelectionDialogue;