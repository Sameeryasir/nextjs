...
import Subaccdialogue from "./subaccdialogue";
...
function Page() {
  ...
  const handleExportToExcel = () => {
    const header = ["Branch", "Tariff", "Sub Account Balance", "Balance"];
    const rows = tableData.map((item) => [
      item.branch,
      item.tariffIndex,
      item.accountBalance,
      item.accountBalance, // Using same value for "Balance"
    ]);
    ...
    saveAs(blob, "sub account-data.xlsx");
  };
      ...
      <div className="max-w-7xl w-full text-left mb-14 space-y-8 px-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
          ...
          <div className="flex flex-wrap gap-2 w-full max-w-4xl">
            ...
            {isDialogOpen && (
              <Subaccdialogue onClose={() => setIsDialogOpen(false)} />
            )}
            <button
              type="button"
        ...
        <table className="w-full min-w-[800px]">
          <thead className="bg-[#FF9900] text-white text-sm font-medium tracking-wide">
            <tr>
              ...
              <th className="p-3 text-left">Sub Account Balance</th>
          ...
          <tbody>
            {tableData.map((item, index) => (
              <tr key={index} className="hover:bg-[#FFE2B7] text-sm transition">
                ...
                <td className="p-3">{item.tariffIndex}</td>
                <td className="p-3">{item.accountBalance}</td>
                <td className="p-3">{item.accountBalance}</td>
              </tr>
            ))}
          </tbody>
