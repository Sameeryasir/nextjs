...
function Page() {
  ...
  const [connCode, setConnCode] = useState("001"); // Default value
  ...
  const handleExportToExcel = () => {
    ...
    const headers = ["Tariff", "Customer Type"];
    const data = tableData.map((row) => [
      row.Tariff, // NOTE: These keys must match the API response
      row.CustomerType,
    ]);
    const worksheet = XLSX.utils.aoa_to_sheet([headers, ...data]);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "CustomersByTariff");
  ...
  const tableHeaders = ["Tariff", "Customer Type"];

  return (
    <div className="min-h-screen bg-white p-6">
      {apiError && <ErrorPopup message={apiError} onClose={() => setApiError(null)} />}
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-semibold text-gray-800">Customers By Tariff</h1>
      ...
      <div className="max-w-7xl w-full text-left mb-14 space-y-8 px-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
          <label className="w-full sm:w-32 text-sm font-medium text-gray-700">Connection Code</label>
          <select value={connCode} onChange={(e) => setConnCode(e.target.value)} className="w-full sm:w-[375px] p-2 border rounded-md bg-gray-50">
            <option value="001">New Connection</option>
            <option value="002">Replacement</option>
            <option value="003">Temp Connection</option>
            <option value="004">BRANCHEMENT TEMPORAIRE</option>
          </select>
        </div>
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            ...
            {isSearching ? (
              ...
              tableData.map((row, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  {/* NOTE: These keys must match the API response */}
                  <td className="p-3">{row.Tariff || 'N/A'}</td>
                  <td className="p-3">{row.CustomerType || 'N/A'}</td>
                </tr>
              ))
            ) : (
