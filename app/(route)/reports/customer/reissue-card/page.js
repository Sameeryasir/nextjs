...
function Page() {
  ...
  const handleExportToExcel = () => {
    ...
    const headers = ["Branch", "Date", "Account No", "Full Name", "Old Card No", "New Card No", "Operator"];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "ReissueCardReport");
    const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
    const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
    saveAs(blob, "reissue_card_report.xlsx");
  };
  ...
  const tableHeaders = ["Branch", "Date", "Account No", "Full Name", "Old Card No", "New Card No", "Operator"];

  return (
    <div className="min-h-screen bg-white p-6">
      {apiError && <ErrorPopup message={apiError} onClose={() => setApiError(null)} />}
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-semibold text-gray-800">Reissue Card Report</h1>
      ...
      <div className="max-w-7xl w-full text-left mb-14 space-y-8 px-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
          <label className="w-full sm:w-32 text-sm font-medium text-gray-700">Card No.</label>
          <input type="text" value={cardNum} onChange={(e) => setCardNum(e.target.value)} className="w-full sm:w-[375px] p-2 border rounded-md bg-gray-50" />
        </div>
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            ...
            {isSearching ? (
              ...
              tableData.map((row, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className="p-3">{row.OldCardNo || 'N/A'}</td>
                  <td className="p-3">{row.NewCardNo || 'N/A'}</td>
