...
// Import the functional Customer Info modal
import CustomerInfoDialogue from "@/app/components/clientdialogue/Dialogue3";
...
function Page() {
  ...
  const handleExportToExcel = () => {
    ...
    const headers = ["Branch", "Date", "Meter Num", "Account No.", "Charge kWH"];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "CustomerPurchases");
    const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
    const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
    saveAs(blob, "customer_purchases.xlsx");
  };
  ...
  const handleClearCustomer = () => setCustomer({ code: "", name: "" });

  const handleSearch = async (page = 1) => {
    if (!customer.code || !dateFrom || !dateTo) {
      alert("Please select a customer and both date fields.");
      return;
    }
    setIsSearching(true);
    setApiError(null);
    ...
    payload.append("COperator", operator);
  ...
  const tableHeaders = ["Branch", "Date", "Meter Num", "Account No.", "Charge kWH"];

  return (
    <div className="min-h-screen bg-white p-6">
      {apiError && <ErrorPopup message={apiError} onClose={() => setApiError(null)} />}
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-semibold text-gray-800">Customer Purchases</h1>
      ...
      <div className="max-w-7xl w-full text-left mb-14 space-y-8 px-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
          <label className="w-full sm:w-32 text-sm font-medium text-gray-700">Customer</label>
          <div className="flex flex-wrap gap-2 w-full max-w-4xl">
            ...
            <button type="button" onClick={() => setIsCustomerDialogOpen(true)} className="w-[50px] h-[40px] bg-[#FF9900] text-white rounded-md flex items-center justify-center hover:brightness-105">...</button>
            {isCustomerDialogOpen && <CustomerInfoDialogue onClose={() => setIsCustomerDialogOpen(false)} onSelect={handleSelectCustomer} />}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
          ...
          <select value={meterType} onChange={(e) => setMeterType(e.target.value)} className="w-full sm:w-[375px] p-2 border rounded-md bg-gray-50">
            <option value="001">General IC Meter</option>
            <option value="002">CPU Card Meter</option>
            <option value="003">Keypad</option>
          </select>
        </div>
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            ...
            {isSearching ? (
              ...
              tableData.map((row, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className="p-3">{row.MeterNum || 'N/A'}</td>
                  <td className="p-3">{row.AccountNo || 'N/A'}</td>
                  <td className="p-3">{row.ChargeKWH || 'N/A'}</td>
                </tr>
              ))
            ) : (
