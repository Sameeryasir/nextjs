...
function Page() {
  ...
  const handleExportToExcel = () => {
    ...
    const headers = ["Branch", "Date", "Account No", "Full Name", "Address", "Cancel Customer Reason", "Operator"];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "MeterCancel");
    const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
    const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
    saveAs(blob, "meter_cancel.xlsx");
  };
  ...
  const tableHeaders = ["Branch", "Date", "Account No", "Full Name", "Address", "Cancel Customer Reason", "Operator"];

  return (
    <div className="min-h-screen bg-white p-6">
      {apiError && <ErrorPopup message={apiError} onClose={() => setApiError(null)} />}
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-semibold text-gray-800">Meter Cancel</h1>
            ...
            {isSearching ? (
              ...
              tableData.map((row, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className="p-3">{row.Address || 'N/A'}</td>
                  <td className="p-3">{row.CancelReason || 'N/A'}</td>
