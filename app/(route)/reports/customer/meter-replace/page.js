...
function Page() {
  ...
  const handleExportToExcel = () => {
    ...
    const headers = ["Date", "Account No", "Full Name", "Address", "Operator"];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "MeterReplace");
    const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
    const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
    saveAs(blob, "meter_replace.xlsx");
  };
  ...
  const tableHeaders = ["Date", "Account No", "Full Name", "Address", "Operator"];

  return (
    <div className="min-h-screen bg-white p-6">
      {apiError && <ErrorPopup message={apiError} onClose={() => setApiError(null)} />}
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-semibold text-gray-800">Meter Replace</h1>
