...
function Page() {
  ...
  const [meterType, setMeterType] = useState("01"); // Default to General IC Meter
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState(""); // Renamed from 'date' for clarity
  ...
  const handleExportToExcel = () => {
    ...
    const headers = ["Account No.", "Change KWH", "Operator", "Date", "Code"];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "VendingByDay");
  ...
  const tableHeaders = ["Account No.", "Change KWH", "Operator", "Date", "Code"];
      ...
      <div className="max-w-7xl w-full text-left mb-14 space-y-8 px-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
          ...
          <select value={meterType} onChange={(e) => setMeterType(e.target.value)} className="w-full sm:w-[375px] p-2 border rounded-md bg-gray-50">
            <option value="01">General IC Meter</option>
            <option value="02">CPU Card Meter</option>
            <option value="03">Keypad</option>
            <option value="04">Solar Meter</option>
          </select>
        </div>
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            ...
            {isSearching ? (
              ...
              tableData.map((row, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className="p-3">{row.Code || 'N/A'}</td>
                </tr>
              ))
            ) : (
