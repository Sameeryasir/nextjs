...
function Page() {
  ...
  const [meterType, setMeterType] = useState(""); // Default to empty
  ...
  const handleExportToExcel = () => {
    ...
    const headers = ["Branch", "Date", "Change KWH"];
    const data = tableData.map((row) => [
      row.Branch,
      row.Date,
      row.ChangeKWH, // Make sure this key matches the API response
    ]);
    const worksheet = XLSX.utils.aoa_to_sheet([headers, ...data]);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "VendingByYear");
    const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
    const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
    saveAs(blob, "vending_by_year.xlsx");
  };
  ...
  const handleSearch = async (page = 1) => {
    ...
    payload.append("ACTION", "34"); // New action for this page
    ...
    payload.append("MeterNum", ""); // API requires this parameter
  ...
  const tableHeaders = ["Branch", "Date", "Change KWH"];

  return (
    <div className="min-h-screen bg-white p-6">
      {apiError && <ErrorPopup message={apiError} onClose={() => setApiError(null)} />}
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-semibold text-gray-800">Vending By Year</h1>
