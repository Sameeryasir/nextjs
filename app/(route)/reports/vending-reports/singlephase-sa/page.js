...
function Page() {
  ...
  const [date, setDate] = useState(""); // Only one date field in the UI
  ...
  const handleExportToExcel = () => {
    ...
    const headers = ["Branch", "Date", "Meter Num.", "Account No.", "Change kWh"];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "SinglePhaseSA");
    const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
    const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
    saveAs(blob, "single_phase_sa.xlsx");
  };
  ...
  const handleSearch = async (page = 1) => {
    if (!branch.code || !date) {
      alert("Please select a branch and a date.");
      return;
    }
    setIsSearching(true);
    setApiError(null);
    ...
    payload.append("ACTION", "44");
    payload.append("PAGE_INDEX", page - 1);
    // The API requires dateFrom and dateTo, but the UI only has one. We'll send the same date for both.
    payload.append("dateFrom", date);
    payload.append("dateTo", date);
    payload.append("FullName", fullName);
    payload.append("COperator", operator);
    payload.append("branchCode", branch.code);
    // These are in the API spec but not in the UI, sending empty for now
  ...
  const tableHeaders = ["Branch", "Date", "Meter Num.", "Account No.", "Change kWh"];

  return (
    <div className="min-h-screen bg-white p-6">
      {apiError && <ErrorPopup message={apiError} onClose={() => setApiError(null)} />}
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-semibold text-gray-800">Single Phase SA</h1>
      ...
      <div className="max-w-7xl w-full text-left mb-14 space-y-8 px-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
          <label className="w-full sm:w-32 text-sm font-medium text-gray-700">Date</label>
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full sm:w-[375px] p-2 border rounded-md bg-gray-50" />
        </div>
            ...
            {isSearching ? (
              ...
              tableData.map((row, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className="p-3">{row.ChangeKWH || 'N/A'}</td>
                </tr>
              ))
            ) : (
