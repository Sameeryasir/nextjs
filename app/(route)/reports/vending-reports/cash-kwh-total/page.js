...
function Page() {
  ...
  const [branchType, setBranchType] = useState("Vending Office");
  ...
  const handleExportToExcel = () => {
    ...
    const headers = ["Region", "Branch", "Date", "Total Sales", "Net Kwh", "Tran Count"];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "VSCashKwhTotal");
    const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
    const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
    saveAs(blob, "VSCashKwhTotal.xlsx");
  };
  ...
  const handleSearch = async (page = 1) => {
    ...
    payload.append("ACTION", "2"); // New action for this page
  ...
  const tableHeaders = ["Region", "Branch", "Date", "Total Sales", "Net Kwh", "Tran Count"];

  return (
    <div className="min-h-screen bg-white p-6">
      {apiError && <ErrorPopup message={apiError} onClose={() => setApiError(null)} />}
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-semibold text-gray-800">VS Cash & kWh Total</h1>
            ...
            {isSearching ? (
              ...
              tableData.map((row, idx) => (
                <tr key={idx} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                  {/* NOTE: These keys (e.g., row.Region) must match the API response */}
                  <td className="p-3">{row.Region || 'N/A'}</td>
                  <td className="p-3">{row.Branch || 'N/A'}</td>
                  <td className="p-3">{row.Date || 'N/A'}</td>
                  <td className="p-3">{row.TotalSales || 'N/A'}</td>
                  <td className="p-3">{row.NetKwh || 'N/A'}</td>
                  <td className="p-3">{row.TranCount || 'N/A'}</td>
                </tr>
              ))
            ) : (
