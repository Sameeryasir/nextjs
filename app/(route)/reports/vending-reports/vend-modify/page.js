...
function Page() {
  ...
  const [meterType, setMeterType] = useState("001"); // Default to General IC Meter
  const [meterNum, setMeterNum] = useState(""); // This was 'code' in the UI
  ...
  const handleExportToExcel = () => {
    ...
    const headers = ["Date", "Account No.", "Full Name", "Operator"];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "VendModify");
  ...
  const handleSearch = async (page = 1) => {
    ...
    // COperator is in the API spec but not in the UI, sending empty for now
    payload.append("COperator", "");
  ...
  const tableHeaders = ["Date", "Account No.", "Full Name", "Operator"];
  ...
  return (
    <div className="min-h-screen bg-white p-6">
      ...
      <div className="max-w-7xl w-full text-left mb-14 space-y-8 px-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
          <label className="w-full sm:w-32 text-sm font-medium text-gray-700">Meter No.</label>
          <input type="text" value={meterNum} onChange={(e) => setMeterNum(e.target.value)} className="w-full sm:w-[375px] p-2 border rounded-md bg-gray-50" />
        </div>
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
